...
class AddPodcastViewModel @Inject constructor(
    ...
    fun addPodcast(podcast: ItunesPodcast, type: String = "podcast") {
        val feedUrl = podcast.feedUrl ?: return
        viewModelScope.launch {
            val id = if (type == "emission") {
                repository.addEmission(name = podcast.name, feedUrl = feedUrl, logoUrl = podcast.artworkUrl)
            } else {
                repository.addPodcast(name = podcast.name, feedUrl = feedUrl, logoUrl = podcast.artworkUrl)
            }
            try {
    ...
    fun addByRssUrl(url: String, type: String = "podcast") {
        viewModelScope.launch {
            _isLoading.value = true
            try {
                val channel = com.prof18.rssparser.RssParser().getRssChannel(url)
                val name = channel.title ?: "Podcast"
                val logo = channel.image?.url ?: channel.itunesChannelData?.image
                val id = if (type == "emission") {
                    repository.addEmission(name = name, feedUrl = url, logoUrl = logo)
                } else {
                    repository.addPodcast(name = name, feedUrl = url, logoUrl = logo)
                }
                repository.refreshPodcast(id)
                _addedEvent.emit(Unit)
            } catch (_: Exception) { }
    ...
    fun isRssUrl(text: String): Boolean {
        return text.trimStart().startsWith("http", ignoreCase = true)
    }
}
